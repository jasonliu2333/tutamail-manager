...
import { IV_BYTE_LENGTH } from "./symmetric/SymmetricCipherUtils.js";
 ...
 */
export function aesEncrypt(key, bytes) {
    return SYMMETRIC_CIPHER_FACADE.encryptBytes(key, bytes);
}
/**
 * @deprecated use aesEncrypt instead
 */
export function aesEncryptConfigurationDatabaseItem(key, bytes, iv) {
 ...
 * @deprecated
 */
export function aes256EncryptSearchIndexEntry(key, bytes) {
    return SYMMETRIC_CIPHER_FACADE.encryptBytesDeprecatedUnauthenticated(key, bytes);
}
/**
 *@deprecated
 */
export function aes256EncryptSearchIndexEntryWithIV(key, bytes, iv) {
    if (iv.length !== IV_BYTE_LENGTH) {
        throw new CryptoError(`Illegal IV length: ${iv.length} (expected: ${IV_BYTE_LENGTH}): ${uint8ArrayToBase64(iv)} `);
    }
 ...
 */
export function aesDecrypt(key, encryptedBytes) {
    return SYMMETRIC_CIPHER_FACADE.decryptBytes(key, encryptedBytes);
}
export function asyncDecryptBytes(key, bytes) {
 ...
 * @deprecated
 */
export function aesDecryptUnauthenticated(key, encryptedBytes) {
