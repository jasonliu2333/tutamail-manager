...
import { bitArrayToUint8Array, FIXED_IV, IV_BYTE_LENGTH, SYMMETRIC_AUTHENTICATION_TAG_LENGTH_BYTES, SYMMETRIC_CIPHER_VERSION_PREFIX_LENGTH_BYTES, uint8ArrayToBitArray, } from "./SymmetricCipherUtils.js";
...
import { SYMMETRIC_KEY_DERIVER } from "./SymmetricKeyDeriver.js";
...
export class AesCbcFacade {
    symmetricKeyDeriver;
    constructor(symmetricKeyDeriver) {
        this.symmetricKeyDeriver = symmetricKeyDeriver;
    }
    /**
     * This should not be called directly! Use SymmetricCipherFacade instead
     */
    encrypt(key, plainText, mustPrependIv, iv, padding, cipherVersion, skipAuthenticationEnforcement = false) {
        ...
        const cipherText = bitArrayToUint8Array(sjcl.mode.cbc.encrypt(new sjcl.cipher.aes(subKeys.encryptionKey), uint8ArrayToBitArray(plainText), uint8ArrayToBitArray(iv), [], padding));
     ...
     * This should not be called directly! Use SymmetricCipherFacade instead
     */
    decrypt(key, cipherText, ivIsPrepended, padding, cipherVersion, skipAuthenticationEnforcement = false) {
        this.tryToEnforceAuthentication(key, cipherVersion, skipAuthenticationEnforcement);
        const subKeys = this.symmetricKeyDeriver.deriveSubKeys(key, cipherVersion);
        let cipherTextWithoutMacAndVersionByte;
        switch (cipherVersion) {
            case SymmetricCipherVersion.UnusedReservedUnauthenticated:
            ...
            case SymmetricCipherVersion.AesCbcThenHmac: {
                ...
                verifyHmacSha256(authenticationKey, cipherTextWithoutMacAndVersionByte, providedMacBytes);
                break;
            }
            default:
        ...
        try {
            return bitArrayToUint8Array(sjcl.mode.cbc.decrypt(new sjcl.cipher.aes(subKeys.encryptionKey), uint8ArrayToBitArray(aesCbcCiphertext), uint8ArrayToBitArray(iv), [], padding));
        }
        catch (e) {
            throw new CryptoError("aes decryption failed", e);
        }
    }
    async decryptAsync(key, cipherText, ivIsPrepended, cipherVersion, skipAuthenticationEnforcement = false) {
    ...
    extractMacAndCipherText(cipherText) {
        const cipherTextWithoutMacAndVersionByte = cipherText.subarray(SYMMETRIC_CIPHER_VERSION_PREFIX_LENGTH_BYTES, cipherText.length - SYMMETRIC_AUTHENTICATION_TAG_LENGTH_BYTES);
    ...
    getIvAndCipherText(ivIsPrepended, cipherTextWithoutMacAndVersionByte) {
        let iv;
        let aesCbcCiphertext;
        if (ivIsPrepended) {
    ...
    tryToEnforceAuthentication(key, cipherVersion, skipAuthenticationEnforcement) {
