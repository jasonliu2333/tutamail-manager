import { FIXED_IV, generateIV, IV_BYTE_LENGTH, keyToUint8Array, uint8ArrayToKey } from "./SymmetricCipherUtils.js";
import { AES_CBC_FACADE } from "./AesCbcFacade.js";
...
export class SymmetricCipherFacade {
     ...
     * @deprecated
     */
    encryptBytesDeprecatedUnauthenticated(key, bytes) {
     ...
     */
    encryptBytesDeprecatedCustomIv(key, bytes, iv) {
     ...
     * @deprecated use encryptBytes instead.
     */
    encryptBytesDeprecatedUnauthenticatedCustomIv(key, bytes, iv) {
     ...
     */
    decryptBytes(key, bytes) {
        return this.decrypt(key, bytes, true);
    }
    async asyncDecryptBytes(key, bytes) {
        if (this.subtleCryptoAvailable) {
     ...
     * @deprecated
     */
    decryptBytesDeprecatedUnauthenticated(key, bytes) {
     ...
     * @deprecated
     */
    decryptKeyDeprecatedUnauthenticated(key, bytes) {
     ...
     * @deprecated
     */
    decryptKeyDeprecatedUnauthenticatedFixedIv(key, bytes) {
    ...
    encrypt(key, plainText, padding, cipherVersion, mustGenerateRandomIv = true, skipAuthenticationEnforcement = false) {
    ...
    decrypt(key, cipherText, padding, hasPrependedIv = true, skipAuthenticationEnforcement = false) {
    ...
    decryptAsync(key, cipherText, hasPrependedIv = true, skipAuthenticationEnforcement = false) {
        const cipherVersion = getSymmetricCipherVersion(cipherText);
        switch (cipherVersion) {
            case SymmetricCipherVersion.UnusedReservedUnauthenticated:
            case SymmetricCipherVersion.AesCbcThenHmac: {
