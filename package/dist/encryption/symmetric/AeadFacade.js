...
export class AeadFacade {
    encrypt(key, plainText, associatedData) {
        ...
        const iv = generateIV();
        const aesCtrCiphertext = bitArrayToUint8Array(sjcl.mode.ctr.encrypt(new sjcl.cipher.aes(key.encryptionKey), uint8ArrayToBitArray(plainText), uint8ArrayToBitArray(iv), []));
    ...
    decrypt(key, cipherText, associatedData) {
        this.validateKeyLength(key);
        ...
        blake3MacVerify(key.authenticationKey, authenticatedData, authenticationTag);
        ...
        return bitArrayToUint8Array(sjcl.mode.ctr.decrypt(new sjcl.cipher.aes(key.encryptionKey), uint8ArrayToBitArray(aesCtrCiphertext), uint8ArrayToBitArray(iv), []));
    }
